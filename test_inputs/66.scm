...
(define (make-num-list start stop step) (if (>= start stop) (list) (append (list start) (make-num-list (+ start step) stop step))))
...
(define (slice list_ start stop) (map (lambda (i) (list-ref list_ i)) (make-num-list start stop 1)))
(slice (list 1 2 3) 0 1)
(slice (list 1 2 3) 0 2)
(define (list-replace list_ replace-ix elt) (map (lambda (i) (if (equal? i replace-ix) elt (list-ref list_ i))) (make-num-list 0 (length list_) 1)))
...
(define (board-replace board r c elt) (map (lambda (r-ix) (let ((test-row (list-ref board r-ix))) (if (equal? r-ix r) (list-replace test-row c elt) test-row))) (make-num-list 0 (height board) 1)))
...
(define (find-first-zero board) (begin (define (helper vals-to-try) (if (empty? vals-to-try) (cons -1 -1) (if (equal? 0 (get-value-cons board (car vals-to-try))) (car vals-to-try) (helper (cdr vals-to-try))))) (helper (all-conss (make-num-list 0 9 1) (make-num-list 0 9 1)))))
