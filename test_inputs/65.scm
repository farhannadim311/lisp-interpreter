...
(define (make-num-list start stop) (if (>= start stop) () (cons start (make-num-list (+ start 1) stop))))
...
(define (initialize-nd dimensions value) (if (equal? (length dimensions) 0) value (map (lambda (_) (initialize-nd (cdr dimensions) value)) (make-num-list 0 (car dimensions)))))
(define (neighbors-nd dimensions coordinates) (if (equal? (length dimensions) 0) (list (list)) (reduce append (map (lambda (suffix) (map (lambda (prefix) (append (list prefix) suffix)) (make-num-list (max 0 (- (car coordinates) 1)) (min (car dimensions) (+ (car coordinates) 2))))) (neighbors-nd (cdr dimensions) (cdr coordinates))) (list))))
