...
(begin
    ...
    (define (list-replace list_ replace-ix elt)
        (map
            ...
            (make-num-list 0 (length list_) 1)
        )
    )
    ...
    (define (make-num-list start stop step)
        (if (>= start stop)
            (list)
            (append (list start) (make-num-list (+ start step) stop step))
        )
    )
    (define all-moves (make-num-list 1 10 1))
    ...
    (define (board-replace board r c elt)
        (map
            ...
            (make-num-list 0 (height board) 1)
        )
    )
    ...
    (define (find-first-zero board)
        (begin
            ...
            (helper (all-conss (make-num-list 0 9 1) (make-num-list 0 9 1)))
        )
    )
