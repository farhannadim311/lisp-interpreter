...
(begin
  ...
  ; make a list from start up until but not including stop
  (define (make-num-list start stop)
    (if (>= start stop)
      (list)
      (cons start (make-num-list (+ start 1) stop))
    )
  )
  ...
  (define (initialize-nd dimensions value)
    (if (equal? (length dimensions) 0)
      ...
      ) (make-num-list 0 (car dimensions)))
    )
  )
        ...
        (map
          (lambda (suffix)
            (map
              ...
              (make-num-list (max 0 (- (car coordinates) 1)) (min (car dimensions) (+ (car coordinates) 2)))
            )
          )
